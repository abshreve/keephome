...
<main class="content-page"><?= the_content() ?>
  <?= get_component('components', 'blog-title-bar', array(
    'title' => ($post_type === 'post') ? "What&#8217;s New" : 'Press Releases',
    ...
    <div class="container">
      <div class="row">
        <div class="col-12">
          ...
          <div class="current mobile-category-selector d-lg-none"><?= $category ? $category->name : 'Latest Articles' ?> <button class="toggle-categories" aria-expanded="false" aria-haspopup="true" aria-controls="category-list">Toggle</button></div>
          <ul id="category-list">
            <li class="<?= $category ? '' : 'current d-none d-lg-inline-block' ?>"><a href="<?= get_permalink(295) ?>">Latest Articles</a></li><?php
            foreach ($categories as $cat) :
              if ($cat->term_id == 1) :
                continue;
              endif; // skip uncategorized ?>
              <li class="<?= $category && $cat->term_id == $category->term_id ? 'current  d-none d-lg-inline-block' : '' ?>"><a href=" <?= get_category_link($cat->term_id) ?>"><?= $cat->name ?></a></li><?php
            endforeach; ?>
          </ul>
        </div>
      </div>
